...
% Fazer ligação com as seções anteriores, observando a particularidade de cada projeção. Então, indicar o método proposto como uma forma de "misturar" essas projeções.
...
That projection combination has the property of not only expanding the amount of input options that a machine learning model can use to decide the \gls{SQI}, but also allows, theoretically, a collaboration among the diverse projection methods. Per example, suppose that we want to assess the signal quality on two different datasets, $D_A$ and $D_B$. To achieve such a goal, we use a machine learning model $\mathcal{M}: \mathcal{T}_{c \times n \times m} \times \mathcal{W} \mapsto \{0,1\}$ which receives an input tensor $T_{c \times n \times m}$ in the set of all tensors with such dimensions $\mathcal{T}_{c \times n \times m}$ and processes it using a weight configuration $W$ in $\mathcal{W}$, giving an \gls{SQI} in $\{0,1\}$. For simplicity, we define $h_{W}(T_{c \times n \times m})=\mathcal{M}(T_{c \times n \times m},W)$ as hypothesis function that results from the usage of the model $\mathcal{M}$ with weights $W$. Then, we select a set of projection method function $p: D_A \cup D_B \mapsto \mathcal{T}_{c \times n \times m}$ in the set $\mathcal{P}=\{p_1,p_2,...,p_c\}$ to convert the signal $X \in D_A \cup D_B$ to a format compatible with $\mathcal{M}$. The combination of both the model $\mathcal{M}$, with hypothesis function $h$, and the projection $p$ results in the function composition $h \circ p: D_A \cup D_B \mapsto \{0,1\}$ that directly maps the input to the \glspl{SQI}.

%000000000000000000000000000

That projection combination has the property of not only expading the ammount of input options that a machine learning model can use to decide the \gls{SQI}, but also allows, theoretically, a colaboration among the diverse projection methods. Per example, supose that we want to assess the quality of the signal on two different datasets, $D_1$ and $D_2$. To achieve such a goal, we feed the signal $X$ to two projection methods, with functions $p_1: \mathbb{R} \mapsto \mathbb{R}$ and $p_2: \mathbb{R} \mapsto \mathbb{R}$, to generate, respectivelly, the tensors $T(X)_1$ and $T(X)_2$ by replicating the projection to each input channel. Then, we feed one of those projection tensors $T(X)_1$ or $T(X)_2$ to a machine learning model $\mathcal{M}$ with possible weights in the set $\mathcal{W}$ and hypothesis functions in the set $\mathcal{H}$. We assume that $(h_0 \in \mathcal{H},W_0 \in \mathcal{W})$ are the hypothesis function and the weights of the initial state of $\mathcal{M}$ and that we can reach all the other functions in $\mathcal{H}\textbackslash\{h_0\}$ and its states in $\mathcal{M}\textbackslash\{W_0\}$ after we train the model $\mathcal{M}$ in that state. Supose as well that $(h_1 \in \mathcal{H}, W_1 \in \mathcal{W})$ and $(h_2 \in \mathcal{H}, W_2 \in \mathcal{W})$ are the unique optimum functions and states of $\mathcal{M}$ for the datasets $D_1$ and $D_2$, respectivelly. In other words, $h_1 \circ p_1$ and $h_2 \circ p_2$ are the optimum functions to predict the \gls{SQI} in the datsets $D_1$ and $D_2$, respectivelly. In that scenario, we can show that the \gls{Mix} method allows the model to achive both optimum functions $h_1 \circ p_1$ and $h_2 \circ p_2$ without changing the projetion method.
